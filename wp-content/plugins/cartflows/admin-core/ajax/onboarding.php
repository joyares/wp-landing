<?php
/**
 * CartFlows Onboarding ajax actions.
 *
 * @package CartFlows
 */
 ...
 * Class Onboarding.
 */
class Onboarding extends AjaxBase {
	...
	/**
	 * Localize nonce for ajax call — routes nonces to the onboarding JS var.
	 *
	...
	public function localize_ajax_action_nonce( $action ) {
		...
		if ( current_user_can( 'manage_options' ) ) {
			...
			add_filter(
				'cartflows_onboarding_localized_vars',
	...
	public function onboarding_exit() {
		...
		if ( ! check_ajax_referer( 'cartflows_onboarding_exit', 'security', false ) ) {
			wp_send_json_error( array( 'message' => $this->get_error_msg( 'nonce' ) ) );
		}
	...
	/**
	 * Update option — onboarding completed.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	...
	public function user_onboarding() {
		...
		if ( ! is_array( $response_body ) || ! isset( $response_body['success'] ) ) {
			wp_send_json_error( array( 'message' => __( 'Unexpected response from the subscription server.', 'cartflows' ) ) );
			return;
		}
		...
		wp_send_json_success(
			array(
				...
				'message' => isset( $response_body['message'] ) && $response_body['message'] ? $response_body['message'] : '',
			)
		);
	}
	...
	public function wizard_activate_plugin() {
		...
		if ( ! array_key_exists( $plugin_init, $installed_plugins ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid plugin path.', 'cartflows' ) ) );
			return;
		}

		$activate = activate_plugin( $plugin_init, '', false, $do_silently );
	...
	public function import_templates( $page_builder, $page = 1 ) {
		...
		if ( ! is_wp_error( $response ) && 200 === wp_remote_retrieve_response_code( $response ) ) {
			...
			wcf()->logger->sync_log( 'API Error: HTTP ' . wp_remote_retrieve_response_code( $response ) );
		}

		return isset( $sites_and_pages['flows'] ) ? $sites_and_pages['flows'] : array();
	}
	...
	public function import_store_checkout() {
		...
		if ( ! empty( $flow['is_instant_checkout'] ) && (bool) $flow['is_instant_checkout'] ) {
			...
			// Get single step Rest API response.
			$response = \CartFlows_API::get_instance()->get_flow( absint( $flow['ID'] ) );
			...
			if ( 'valid' !== $license_status ) {
				...
				$cf_pro_status = OnboardingCore::get_instance()->get_plugin_status( 'cartflows-pro/cartflows-pro.php' );
			...
			wcf()->logger->import_log( '✓ Flow Created! Flow ID: ' . $new_flow_id . ' - Remote Flow ID - ' . absint( $flow['ID'] ) );
